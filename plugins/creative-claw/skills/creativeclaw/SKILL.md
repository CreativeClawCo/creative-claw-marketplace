...
5. **Respect cost and approval boundaries.** Explain the selected model and material settings before an expensive batch, long clip, or multi-shot production. When the user asks about cost, balance, affordability, or fitting a request into a budget, call `estimate_generation` with the exact planned model and parameters when it is exposed. Treat its result as an estimate; the final cost is confirmed after generation finishes. Do not require this preflight for every generation. Use other credit tools only when present.

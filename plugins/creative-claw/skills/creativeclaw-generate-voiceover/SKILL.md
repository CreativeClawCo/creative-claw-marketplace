...
4. Choose `speech/elevenlabs-v2` for steady narration and identity-focused existing clones in supported languages; choose `speech/elevenlabs-v3` for audio tags, acted dialogue, or languages outside v2. Choose `speech/cartesia-sonic` for fast natural stock or Character speech with direct emotion, speed, and volume controls. Fetch current settings with `get_model_params`. Use `list_models({ category: "speech" })` only when discovering alternatives. Estimate with `operation: "speech"` only for user-requested cost or budget help.
5. Choose a stock `voice_id`, or pass a saved `character_id` whose consented provider clone should speak. For Cartesia, any exact public Voice Library ID is supported and `get_model_params` returns a curated shortlist.
...
- `voice_id` selects a stock voice. `character_id` selects a saved Character's cloned provider voice. Do not combine them.

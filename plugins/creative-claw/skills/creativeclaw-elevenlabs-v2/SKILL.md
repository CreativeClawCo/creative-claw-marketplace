---
name: creativeclaw-elevenlabs-v2
description: "Produce steady professional narration with ElevenLabs Multilingual v2 in Creative Claw. Use for corporate presenters, explainers, e-learning, audiobooks, and identity-focused auditions of existing voice clones; also when the user explicitly requests v2. Use v3 for expressive audio tags or languages outside v2."
---

# Creative Claw — ElevenLabs Multilingual v2

Use `speech/elevenlabs-v2` for controlled, natural narration. It uses the same existing ElevenLabs voice or Character as v3. Selecting v2 does not create or upgrade a Professional Voice Clone (PVC); Instant Voice Cloning and PVC are separate voice-creation methods.

## Choose deliberately

| Need | Model |
| --- | --- |
| Corporate presenter, restrained explainer, e-learning, audiobook | Multilingual v2 |
...
An explicit user model choice wins. V2 is our workflow recommendation for steady narration, not a provider guarantee of better similarity. Do not recreate a clone to change TTS models. Creating another clone can alter its sound even from identical source audio.
...
3. Reuse the requested `character_id` for an existing consented clone, or choose a catalog `voice_id` matching language, accent, age and energy. If cloning is needed, use `creativeclaw-clone-voice` with the speaker's permission. Do not silently substitute a stock voice.
...
Top level: `model`, `text`, `voice_id` **or** `character_id`, optional `speed`.
...
For a catalog voice, replace `character_id` with a discovered `voice_id`. Do not pass source `audio_url` to ElevenLabs TTS. No `agentic_prompting` parameter is exposed by `generate_speech`.
